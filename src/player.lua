player = world:newBSGRectangleCollider(234, 184, 12, 8, 2)
player.x = 0
player.y = 0
player.dir = "down"
player.dirX = 1
...
function player:update(dt)
    ...
    if player.state == 0 then
        ...
        if data.item['right'] == "bow" then
            if controlPad and controlPad:isDown(6) then
                player.aiming = true
            elseif player.aiming then
                player:useBow()
                player:setDirFromVector(player.bowVec)
...
--[[ function player:jumpGravity()

    if player.state ~= 0 then
        player:addToBuffer("jump")
        return
    end

    if pause.active then player:justIdle() return end

    player.ground = player:getY()
